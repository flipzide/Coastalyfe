 ...
 * <a href="http://www.powerset.com/">http://www.powerset.com/</a>
...
/* ********  E N C O D I N G   M E T H O D S  ******** */
     ...
     * @since 2.0
     */
    public static String encodeBytes(byte[] source, int off, int len, int options) throws java.io
            .IOException {
    ...
    public static byte[] encodeBytesToBytes(byte[] source) {
        ...
        try {
            ...
            assert false : "IOExceptions only come from GZipping, which is turned off: " + ex
                    .getMessage();
        }
        return encoded;
    }
     ...
     * @since 2.3.1
     */
    public static byte[] encodeBytesToBytes(byte[] source, int off, int len, int options) throws
            java.io.IOException {
        ...
        if (off + len > source.length) {
            throw new IllegalArgumentException(
                    String.format("Cannot have offset of %d and length of %d with array of length" +
                            " %d", off, len, source.length));
        }   // end if: off < 0
...
//        }
        ...
        for (i = off; i < off + len; i++) {  // Loop through source
            ...
            if (sbiDecode >= WHITE_SPACE_ENC) {
                ...
                throw new java.io.IOException(String.format(
                        "Bad Base64 input character decimal %d in array position %d", ((int)
                                source[i]) & 0xFF, i));
            }   // end else: 
    ...
    public static String encodeFromFile(String filename)
        ...
        try {
            ...
            // () for math on small files (v2.2.1); Need +1 for a few corner cases (v2.3.5)
            int length = 0;
            int numBytes = 0;
